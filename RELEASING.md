...
8. **Smoke test the published package**
   ...
   laravel new sandbox
   cd sandbox
   composer require Yami/assessments:^X.Y
   php artisan vendor:publish --tag=assessments-config
   php artisan migrate
   ```

9. **Document any host-app steps**
   - Update the host repository’s `composer.json` constraint (`Yami/assessments": "^X.Y"`).
   - Run `composer update Yami/assessments` inside the host app.
