...
Reusable Assessments (question bank + exams) module extracted from the Amryami HR platform and packaged for Laravel 12 projects.
...
1. Require the package in your Laravel application:
   ```bash
   composer require amryami/assessments:^1.0
   ```
...
4. Seed the default permissions / demo data if desired:
   ```bash
   php artisan db:seed --class="Fakeeh\\Assessments\\Database\\Seeders\\AssessmentsPermissionSeeder"
   ```
...
- `assessments.models.category` / `assessments.models.user` (or env `ASSESSMENTS_MODEL_CATEGORY/USER`) must be set to the host app's Eloquent classes if the defaults cannot be auto-discovered by `Fakeeh\Assessments\Support\ModelResolver`.
...
- Package controllers now consume custom FormRequest classes (`Fakeeh\\Assessments\\Support\\FormRequest`) which proxy through to any host base request when present (topics, exams, presets, answer sets UI/API, propagation, reviews, questions, candidate attempts), and API responses lean on dedicated resources (answer sets, attempt start, exam preview).
...
- Temporary class aliases are defined in `src/helpers.php`; remove them once consumers switch fully to the `Fakeeh\Assessments\` namespace.
